...
import { ColorPallet } from "@facilsoft_theme_backend/js/color_pallet";
...
patch(NavBar.prototype, {
    async setup(parent, menuData) {
        ...
        $(document).on('click', '.o_navbar_apps_menu .facilsoft-menu-group-list', function (ev) { self._facilsoftMenuGroupList(ev) });
        ...
        $(document).on('click', '.o_navbar_apps_menu .submenu-link', function (ev) { self._facilsoftMainGroup(ev) });
        $(document).on('click', '.o_navbar_apps_menu .facilsoft-submenu-group', function (ev) { self._facilsoftSubMenuGroup(ev) });
    ...
    _all_apps_menu_data: async function () {
        ...
        await rpc('/get/irmenu/icondata', {
            ...
            $.each(menu_data, function (key, value) {
                ...
                var facilsoft_app_group = rec["facilsoft_app_group"]
                ...
                value.facilsoft_app_group_id = current_record.facilsoft_app_group_id
                value.facilsoft_app_group = facilsoft_app_group
                value.app_menu_list = JSON.parse(current_record.app_menu_list)

                if ($('body').hasClass('top_menu_horizontal')) {
                    const horizontalMenuContainer = $(".facilsoft-app-group");
                    if (!horizontalMenuContainer.data('menu-rendered')) {
                        const horizontalMenuTemplate = $(renderToElement("facilsoft_theme_backend.AppMenuGroup", {
                            group_info: value.facilsoft_app_group,
                            menu_info: menu_data,
                    ...
                    const interval = setInterval(() => {
                        ...
                        if (verticalMenuContainer.length) {
                            ...
                            if (!verticalMenuContainer.data('menu-rendered')) {
                                const verticalMenuTemplate = $(renderToElement("facilsoft_theme_backend.AllAppsMenus", {
                                    group_info: value.facilsoft_app_group,
                                    menu_info: menu_data,
                                    getMenuItemHref: self.getMenuItemHref,
                ...
                if (current_record.use_icon) {
                    if (current_record.icon_class_name) {
                        ...
                        var icon_image = "<img class='img img-fluid' src='/facilsoft_theme_backend/static/description/bizople-icon.png' />"
                    }
    ...
    _facilsoftMenuGroupList: function (ev) {
        ev.preventDefault();
        var $blurOverlay = $('#blur-overlay');
        var $backgroundOverlay = $('.top-menu-vertical-mini');
    ...
    _onMenuGroupClick: function (ev) {
        ...
        if ($('body').hasClass('top_menu_horizontal')) {
            var $target = $(ev.currentTarget).siblings('.facilsoft-submenu-group');
            
            if ($target.hasClass('active')) {
                $target.removeClass('active').hide();
            } else {
                $('.facilsoft-submenu-group').not($target).removeClass('active').hide();
    ...
    _facilsoftMainGroup: function (ev) {
        if ($('body').hasClass('top_menu_horizontal')) {
            // $('.appdrawer_section').removeClass('toggle');
    ...
    _facilsoftSubMenuGroup: function (ev) {
        if ($('body').hasClass('top_menu_horizontal')) {
            $('.facilsoft-submenu-group').addClass('d-none').removeClass('active');
        }
    },
    ...
    addmultitabtags: function (ev) {
        ...
        rpc('/get/mutli/tab', {}).then(function (rec) {
            if (rec) {
                ...
                var facilsoftstoredActionId = sessionStorage.getItem("facilsoft_current_action_id");
                var facilsoftstoredAction = sessionStorage.getItem("facilsoft_current_action");

                if (facilsoftstoredActionId) {
                    var TabDiv = $('.multi_tab_section .multi_tab_div');
                    var ActiveMenu = TabDiv.find('a[data-action-id="' + facilsoftstoredActionId + '"]');
    ...
    addconfiguratorclass: function () {
        rpc('/get/model/record').then(function (rec) {
            ...
            if (rec.record_dict[0].font_size) {
                loadCSS(`/facilsoft_theme_backend/static/src/scss/font_sizes/${rec.record_dict[0].font_size}.css`);
            }
            ...
            if (rec.record_dict[0].top_menu_position === "top_menu_vertical_mini_2") {
                ...
                const imageMap = {
                    top_menu_vertical_bg1: {
                        ...
                        fallback: "/facilsoft_theme_backend/static/description/top-menu-v2-bg1.jpg"
                    },
                    top_menu_vertical_bg2: {
                        field: "vertical_mini_bg_image_two",
                        fallback: "/facilsoft_theme_backend/static/description/top-menu-v2-bg2.jpg"
                    },
                    top_menu_vertical_bg3: {
                        field: "vertical_mini_bg_image_three",
                        fallback: "/facilsoft_theme_backend/static/description/top-menu-v2-bg3.jpg"
                    },
                    top_menu_vertical_bg4: {
                        field: "top_menu_custom_bg_vertical_mini_2",
                        fallback: "/facilsoft_theme_backend/static/description/top-menu-v2-bg4.jpg"
                    }
                };
            ...
            if (rec.record_dict[0].menu_bg_image) {
                ...
                $(".new_systray").attr("style", "background-image: url('/facilsoft_theme_backend/static/description/header_vertical_mini.svg')");
                $(".top_menu_horizontal.top_menu_vertical_mini_mobile .o_main_navbar").attr("style", "background-image: url('/facilsoft_theme_backend/static/description/header_vertical_mini.svg')");
            }
                ...
                if (fontName && fontName !== "Unknown Font") {
                    ...
                    rpc('/add/google/font', { 
                        ...
                        if (data.status == 'success') {
                            ...
                            const NewFontFamilyAdd = $(renderToElement("facilsoft_theme_backend.AddNewFontFamily", {
                                fontId: data.id,
                                fontName: data.name,
                            }));  
    ...
    _searchPages: function () {
        ...
        if (searchvals_option === "menu") {
            ...
            this.$search_modal_results.empty().append(renderToFragment('facilsoft_theme_backend.MenuSearchResults', {
                results: results,
                widget: this,
            }));
        }
            ...
            options.each(function() {
                ...
                rpc('/get/records/data', {
                    ...
                    if (rec) {
                        ...
                        search_modal_results.empty().append(renderToFragment('facilsoft_theme_backend.RecordSearchResults', {
