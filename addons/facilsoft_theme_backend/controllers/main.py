...
from odoo.addons.web.controllers.webmanifest import WebManifest as facilsoftWebManifest
...
class BackendConfigration(http.Controller):
    ...
    def selected_pallet_data(self, **kw):
        ...
        response = request.render(
            "facilsoft_theme_backend.template_backend_config_data", values)
    ...
    def get_irmenu_icondata(self, **kw):
        ...
        facilsoft_app_group = request.env['facilsoft.app.group'].sudo().search_read([], fields=['id', 'name', 'group_menu_icon', 'sequence'])
        facilsoft_app_group = sorted(facilsoft_app_group, key=lambda group: group['sequence'])

        irmenu = request.env['ir.ui.menu'].sudo().search([('id', 'in', kw.get('menu_ids'))])

        app_menu_dict = {}
        processed_facilsoft_app_group_ids = set()

        menu_data = []
        groups = request.env['facilsoft.app.group'].sudo().search([])
        ...
        menu_list = []
        for menu in irmenu:
            if not menu.facilsoft_app_group_id:
                menu_list.append(menu.id)
        app_menu_list = str(menu_list) if menu_list else "[]"
        for menu in irmenu:
            if menu.facilsoft_app_group_id:
                facilsoft_app_group_id = menu.facilsoft_app_group_id[0].id

                if facilsoft_app_group_id not in processed_facilsoft_app_group_ids:
                    facilsoft_app_group_data = request.env['facilsoft.app.group'].sudo().search_read(
                        [('id', '=', facilsoft_app_group_id)], 
                        fields=['id', 'name', 'sequence', 'group_menu_icon', 'group_menu_list_ids', 'use_group_icon', 'group_icon_class_name']
                    )
                    if facilsoft_app_group_data:
                        app_menu_dict.setdefault('facilsoft_app_group', []).append(facilsoft_app_group_data[0])
                        processed_facilsoft_app_group_ids.add(facilsoft_app_group_id)
        ...
        if 'facilsoft_app_group' in app_menu_dict:
            app_menu_dict['facilsoft_app_group'] = sorted(app_menu_dict['facilsoft_app_group'], key=lambda group: group['sequence'])
    ...
    def show_user_todo_list(self, **kw):
        ...
        response = request.render("facilsoft_theme_backend.to_do_list_template", values)
    ...
    def create_todo(self, **kw):
        ...
        if user_id and (note_title or note_description):
            ...
            note_content = request.env['ir.ui.view']._render_template(
                "facilsoft_theme_backend.to_do_list_content_template", {
                    'note': todo_record,
                    'today_date': today_date_with_offset,
                    'user_tz_offset_time': user_tz_offset_time,
                }
    ...
    def ThemecolorParameterCheck(self, uid,**post):
        module_obj = request.env['ir.module.module'].sudo().search([('name','=','facilsoft_theme_backend'),('state','=','installed')])
        if module_obj:
            if post.get('color_data'):
                ...
                if view_obj:
                    ...
                    if view_color == -1:
                        return {
                            'code':201,
                            'message':'facilsoft Theme is not installed in your Odoo'
                            }
                else:
                    return {
                            'code':201,
...
class WebManifest(facilsoftWebManifest):
    def _icon_path(self):
        return 'facilsoft_theme_backend/static/src/image/loader_2.gif'
